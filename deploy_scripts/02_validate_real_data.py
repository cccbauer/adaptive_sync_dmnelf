"""
02_validate_real_data.py

Comprehensive validation with:
- Multi-band PLV (alpha, beta_low, beta_high)
- DiFuMo group networks + personal networks
- Auto bad channel detection/interpolation

Usage:
    python deploy_scripts/02_validate_real_data.py --subject sub-dmnelf001 --condition feedback
...
r_target = str(cond_params['agent']['r_target'])
input_type = cond_params['input_type']
input_mag = str(cond_params['input_magnitude'])
primary_band = cond_params['freq_band']

print(f"\nValidating: {SUBJECT}, {CONDITION}, primary band={primary_band}")
...
CLUSTER_SCRIPT = [
    ...
    '# ========== Load EEG ==========',
    'print("\\n1. Loading EEG...")',
    'eeg_base = Path("' + str(config.CLUSTER_PREPROCESSED) + '") / SUBJECT / "' + config.SESSION + '" / "eeg"',
    ...
    '',
    '# ========== Simulate Kuramoto ==========',
    'print("\\n2. Simulating Kuramoto (K=' + k_coupling + ')...")',
    ...
    '',
    '# Personal parcels',
    'personal_dir = Path("' + str(config.PERSONAL_PARCEL_DIR) + '")',
    ...
    'print("SUMMARY")',
    'print("=" * 70)',
    'print(f"Simulated R(t): {np.mean(sync_sim):.3f} (target: ' + r_target + ')")',
    ...
    '',
    '# Save',
    'output_dir = Path("' + str(config.CLUSTER_RESULTS) + '") / "validation"',
...
SLURM_SCRIPT = [
    '#!/bin/bash',
    '#SBATCH --job-name=val_' + SUBJECT[-3:],
    '#SBATCH --partition=short',
    '#SBATCH --time=00:30:00',
    '#SBATCH --cpus-per-task=2',
    '#SBATCH --mem=8G',
    '#SBATCH --output=' + str(config.CLUSTER_LOGS / f'val_{SUBJECT}_{CONDITION}_%j.out'),
    '#SBATCH --error=' + str(config.CLUSTER_LOGS / f'val_{SUBJECT}_{CONDITION}_%j.err'),
    '',
    'source /shared/EL9/explorer/anaconda3/2024.06/etc/profile.d/conda.sh',
    'python ' + str(config.CLUSTER_SCRIPTS / f'02_validate_{CONDITION}_cluster.py'),
]

def deploy():
    # Write cluster script with condition-specific filename
    local_file = config.LOCAL_SCRIPTS / f"02_validate_{CONDITION}_cluster.py"
    ...
    # Copy to cluster with condition-specific name
    cluster_py = config.CLUSTER_SCRIPTS / f"02_validate_{CONDITION}_cluster.py"
    subprocess.run(config.scp_to_cluster(local_file, cluster_py), shell=True, check=True)
    
    # Write and copy SLURM script
    slurm_file = config.LOCAL_SCRIPTS / f"02_validate_{CONDITION}.sh"
    with open(slurm_file, 'w') as f:
        f.write('\n'.join(SLURM_SCRIPT))
    
    cluster_sh = config.CLUSTER_SCRIPTS / f"02_validate_{CONDITION}.sh"
    subprocess.run(config.scp_to_cluster(slurm_file, cluster_sh), shell=True, check=True)
    print("✓ Deployed")
    
    # Submit job
    cmd = f"sbatch {cluster_sh}"
...
if __name__ == "__main__":
    print("=" * 70)
    print(f"Validation: {SUBJECT} - {CONDITION}")
    print("=" * 70)
    deploy()
